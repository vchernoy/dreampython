...
## The `+` Operation in Python (May 5)

In Python, the `+` operation can be applied not only to numbers, but also to strings, lists, and even tuples. Examples:
```py
print(100 + 5)
print(100. + 5.)
print('100' + '5')
...
We can format the previous script as a table and a loop that will apply + to each pair of values from the table.
```py
tab = (
    (100, 5), 
    (100., 5.), 
...
We get output with indication of result types (int, float, str, list, tuple):
```
100    + 5    = 105      -- type: int
...
## The `*` Operation in Python (May 11)

In Python, you can multiply not only numbers by a number, but also strings, lists, and tuples. Multiplying something by 4 means adding "that thing" four times. Examples:
```py
print(20 * 4)
print(20. * 4)
print('20' * 4)
