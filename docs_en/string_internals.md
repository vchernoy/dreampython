...
In previous parts, part of the TStr class (strings based on tuples) was implemented. 
Here's what we got:
...
Now let's consider how to implement the `join` method. 
...
Here we have a problem, since `__init__` expects a string, not a tuple. 
Let's fix `__init__`:
